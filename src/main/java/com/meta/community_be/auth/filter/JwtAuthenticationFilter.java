...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        // 4. JWT 토큰에서 사용자 계정(Username)이 존재하며, SpringSecurityContext 공간에 인증 정보가 존재하지 않는 경우에만 인증 진행
        if (username != null || SecurityContextHolder.getContext().getAuthentication() == null) {
            // 5. UserDetailsService 통해 사용자계정(Username)을 기반으로 UserDetails 객체 로드
            UserDetails userDetails = userDetailsService.loadUserByUsername(username);
            // 6. 토큰 유효성 검사(사용자 이름 일치, 토큰 만료 여부)
            if (jwtUtil.validateToken(jwtToken, userDetails)) {
                // 7. 토큰이 유효한 경우, SpringSecurityContext에 인증 정보 설정
                UsernamePasswordAuthenticationToken authToken = new UsernamePasswordAuthenticationToken(
                        userDetails, // 인증된 사용자 객체(UserDetails)
                        null, // 자격증명은 이미 토큰으로 검증되었기 때문에 null로 설정
                        userDetails.getAuthorities() // 사용자 권한 목록 중 하나
                );

                // 8. 요청에 대한 웹 인증 세부 정보(IP, 세션 ID 등) 설정
                authToken.setDetails(new WebAuthenticationDetailsSource().buildDetails(request));

                // 9. SecurityContextHolder에 생성된 인증 토큰을 설정
